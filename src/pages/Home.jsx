...
const Home = ({categories}) => {
    ...
    const latestIds   = useMemo(() => new Set(latestThree.map(r => r.id)), [latestThree]);
    ...
    return (
        <div>
            ...
            {categories.map(cat => {
                const prettyName = cat.name.charAt(0).toUpperCase() + cat.name.slice(1);
                const recipesForThisCat = recipes.filter(r => r.category_id === cat.id);
                const filtered = recipesForThisCat.filter(r => !latestIds.has(r.id));
